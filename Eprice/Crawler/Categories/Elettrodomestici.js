...
//prendo le categorie dal sitemap
    const links = $('#main ul li').map((i, sitemap) =>{
      const $elettrodomestici = $(sitemap).find('a[href^="/s/elettrodomestici/"]').attr('href');
      if($elettrodomestici !== undefined) {
        //aggiungo ?fd=1 alla fine di ogni url per preparare a contare le pagine
        $elettrodomestici.charAt(0) !== '/' ? verifiedUrl = website + '/' + $elettrodomestici + '?fd=1' : verifiedUrl = website + $elettrodomestici + '?fd=1'
        return verifiedUrl;
      }
    }).get();
    ...
    await Promise.all(links.map(async (link) => {
      try {
        ...
        fs.writeFile('../../Jsons/Categories/elettrodomestici.json', data, (err) => {
            if (err) throw err;
        });
    ...
    console.log('Elettrodomestici is finshed');
  } catch (e) {
