...
(async function () {
  try {
    ...
    //prendo la categoria  dal sitemap
    const links = $('#main ul li').map((i, sitemap) =>{
      const $informatica = $(sitemap).find('a[href^="/s/informatica"]').attr('href');
      if($informatica !== undefined) {
        //aggiungo ?fd=1 alla fine di ogni url per preparare a contare le pagine
        $informatica.charAt(0) !== '/' ? verifiedUrl = website + '/' + $informatica + '?fd=1' : verifiedUrl = website + $informatica + '?fd=1'
        return verifiedUrl;
      }
    }).get();
    ...
    await Promise.all(links.map(async (link) => {
      try {
        ...
        fs.writeFile('../../Jsons/Categories/informatica.json', data, (err) => {
            if (err) throw err;
        });
    ...
    console.log('Informatica is finished');
  } catch (e) {
