...
(async function () {
  try {
    ...
    fs.readFile('../../Jsons/Categories/elettrodomestici.json', async (err, data) => {
      if (err) throw err;
      ...
      await Promise.all(singleUrls.map(async (singleUrl) => {
        try{
          ...
          fs.writeFile('../../Jsons/Items/elettrodomestici.json', data, (err) => {
              if (err) throw err;
          });
      ...
      console.log('Items elettrodomestici s finished');
    });
  } catch (e) {
